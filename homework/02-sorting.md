...
因为最优情况就是，每次对数组做 SPLIT 时，恰好总是选择了那个「DELIMITER」，使得「小于 DELIMITER」和「大于 DELIMITER」的元素数量基本一致；这样的话可以在 $O(n\log n)$ 时间内达成。反过来，比较差的情况就是 DELIMITER 非常接近输入数组中的最大值或者最小值，这会导致增加很多无意义的分割次数。

所以，采用中项作为 DELIMITER，可以尽量使得被 SPLIT 两边的长度差不多，这样的效果是最好的。
...
1. 在数组开头插入一个 `0` 元素（时间花费：$O(n)$）
2. 使用 SPLIT 算法，并规定 `0` 作为 DELIMITER（时间花费：$O(n)$）
3. 遍历数组，并删除其中第一个为 `0` 的元素（时间花费：$O(n)$）
