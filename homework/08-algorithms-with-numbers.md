...
回答：满足 $x = \sqrt{kp + 1}$ 且为整数的 $x$。

只有 $x \equiv 1$（此时 $k = 0$ 或 $k = p + 2$）或 $x \equiv p - 1$（此时 $k = p - 2$）。

要构成一个完全平方式，固定了 $c$，那么要么凑出 $\sqrt{(x - \sqrt{c})^2}$，要么就凑出 $\sqrt{(x + \sqrt{c})^2}$，要么就 $\sqrt{c}$。只有这三种情况。其中第二种和第三种取模运算之後一样。

---

2. 证明对素数 $p$ 来说，$(p - 1)! \equiv -1 \pmod{p}$。

证明：先考虑 $p \gt 2$ 地非平凡情况。
