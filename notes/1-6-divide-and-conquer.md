...
有一种策略是：不断尝试用不同的「划分依据」对原数组进行划分；如果正好将其分为一半，那么中位数找到，结束。如果分为一大一小两半，那么抛弃其中的、不包含所求的那一半，对剩余的一半继续寻找第 $k$ 大的元素。

> 例如，本来我们想寻找第 $k$ 大的数字；而在这里我们抛弃了较小的 $t$ 个数字；那么问题不变，仍然是寻找第 $k$ 大的数字；但如果我们抛弃了较大的 $t$ 个数字（此时必然有 $t \lt k$），那么问题就变成了寻找第 $k - t$ 大的元素了。
